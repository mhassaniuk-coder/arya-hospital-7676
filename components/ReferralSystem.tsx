        ...
        <div className="space-y-6 animate-fade-in">
             <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-slate-500">Manage transfers between hospitals.</p>
                </div>
                <button 
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {referrals.map(ref => (
                            <tr key={ref.id} className="hover:bg-slate-50 transition-colors">
                                <td className="px-6 py-4 font-mono text-xs text-slate-500">{ref.id}</td>
                                <td className="px-6 py-4">
                                    <span className={`flex items-center gap-1 w-fit px-2 py-1 rounded text-xs font-bold ${ref.direction === 'Outbound' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'}`}>
                                        {ref.direction === 'Outbound' ? <ArrowRight size={12} /> : <ArrowLeft size={12} />}
                                        {ref.direction}
                                    </span>
                                </td>
                                <td className="px-6 py-4 font-medium text-slate-900">{ref.patientName}</td>
                                <td className="px-6 py-4 text-slate-600">{ref.hospital}</td>
                                <td className="px-6 py-4 text-slate-600 text-sm">{ref.reason}</td>
                                <td className="px-6 py-4">
                                     <span className={`px-2 py-1 rounded-full text-xs font-bold ${ref.status === 'Accepted' ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-700'}`}>
                                         {ref.status}
                                     </span>
                                </td>
                            </tr>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 m-4 animate-scale-up">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Target Hospital</label>
                                <input 
                                    required
                                    type="text" 
                                    className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 outline-none"
