    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Legal / MLC</h1>
                    <p className="text-slate-500 dark:text-slate-400">Medico-Legal Cases and documentation.</p>
                </div>
                ...
                <div className="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-slate-800 dark:to-slate-900 rounded-2xl p-6 border border-purple-200 dark:border-purple-800">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-lg font-semibold text-purple-900 dark:text-purple-100">AI Contract Management</h3>
                        ...
                        <button onClick={() => setShowContractPanel(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-white">✕</button>
                    </div>

                    {/* Expiry Alerts */}
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-purple-100 dark:border-purple-900/30 mb-4">
                        <h4 className="font-semibold text-slate-800 dark:text-white mb-3">Contract Expiry Alerts</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {contractManagement.data.expiryAlerts.map((alert, idx) => (
                                <div key={idx} className={`p-3 rounded-lg ${alert.priority === 'Critical' ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/30' :
                                        alert.priority === 'High' ? 'bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-900/30' :
                                            'bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-900/30'
                                    }`}>
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="font-medium text-slate-800 dark:text-slate-200">{alert.contractName}</span>
                                        <span className={`text-xs px-2 py-0.5 rounded ${alert.daysUntilExpiry <= 30 ? 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300' :
                                                alert.daysUntilExpiry <= 60 ? 'bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300' :
                                    ...
                                    <div className="flex items-center gap-2 mt-2">
                                        ...
                                        <span className="text-xs text-slate-500 dark:text-slate-400">Action: {alert.recommendedAction}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-purple-100 dark:border-purple-900/30 mb-4">
                        <h4 className="font-semibold text-slate-800 dark:text-white mb-3">Renewal Recommendations</h4>
                        {contractManagement.data.renewalRecommendations.map((rec, idx) => (
                            <div key={idx} className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg mb-2">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="font-medium text-slate-800 dark:text-slate-200">{rec.contractName}</span>
                                    <span className={`text-xs px-2 py-0.5 rounded ${rec.recommendation === 'Renew' ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' :
                                            rec.recommendation === 'Renegotiate' ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300' :
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-purple-100 dark:border-purple-900/30">
                        <h4 className="font-semibold text-slate-800 dark:text-white mb-3">Vendor Insights</h4>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            {contractManagement.data.vendorInsights.map((vendor, idx) => (
                                <div key={idx} className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="font-medium text-slate-700 dark:text-slate-300">{vendor.vendor}</span>
                                        <span className={`text-xs px-2 py-0.5 rounded ${vendor.riskLevel === 'Low' ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' :
                                                vendor.riskLevel === 'Medium' ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300' :
                                                    'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300'
                                            }`}>
            ...
            {showPolicyPanel && policyManagement.data && (
                <div className="bg-gradient-to-r from-teal-50 to-cyan-50 dark:from-teal-900/20 dark:to-cyan-900/20 rounded-2xl p-6 border border-teal-200 dark:border-teal-800">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            <RefreshCw className="text-teal-600 dark:text-teal-400" size={20} />
                            <h3 className="text-lg font-semibold text-teal-900 dark:text-teal-100">AI Policy Management</h3>
                        ...
                        <button onClick={() => setShowPolicyPanel(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-white">✕</button>
                    </div>
                    ...
                    {policyManagement.data.alerts.length > 0 && (
                        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-teal-100 dark:border-teal-900/30 mb-4">
                            <h4 className="font-semibold text-slate-800 dark:text-white mb-3">Policy Alerts</h4>
                            {policyManagement.data.alerts.map((alert, idx) => (
                                <div key={idx} className={`p-3 rounded-lg mb-2 ${alert.severity === 'Critical' ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/30' :
                                    ...
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="font-medium text-slate-800 dark:text-slate-200">{alert.policy}</span>
                                        <span className={`text-xs px-2 py-0.5 rounded ${alert.severity === 'Critical' ? 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300' :
                                    ...
                                    <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">Action: {alert.action}</p>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {/* Gap Analysis */}
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-teal-100 dark:border-teal-900/30 mb-4">
                        <h4 className="font-semibold text-slate-800 dark:text-white mb-3">Compliance Gap Analysis</h4>
                        {policyManagement.data.gapAnalysis.map((gap, idx) => (
                            <div key={idx} className="p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg mb-2">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="font-medium text-slate-800 dark:text-slate-200">{gap.policyName}</span>
                                    <span className={`text-xs px-2 py-0.5 rounded ${gap.severity === 'Critical' ? 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300' :
                                            gap.severity === 'High' ? 'bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300' :
                                                'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300'
                                        }`}>{gap.severity}</span>
                                </div>
                                <p className="text-sm text-slate-600 dark:text-slate-400">{gap.description}</p>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-teal-100 dark:border-teal-900/30">
                        <h4 className="font-semibold text-slate-800 dark:text-white mb-3">Upcoming Policy Reviews</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {policyManagement.data.reviewSchedule.map((review, idx) => (
                                <div key={idx} className={`p-3 rounded-lg ${review.status === 'Overdue' ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/30' :
                                        review.status === 'Due Soon' ? 'bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/30' :
                                            'bg-slate-50 dark:bg-slate-700/30'
                                    }`}>
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="font-medium text-slate-700 dark:text-slate-300">{review.policy}</span>
                                        <span className={`text-xs px-2 py-0.5 rounded ${review.status === 'Overdue' ? 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300' :
                                                review.status === 'Due Soon' ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300' :
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                        {MOCK_CASES.map(c => (
                            <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                <td className="px-6 py-4 font-mono text-sm text-slate-600 dark:text-slate-400">{c.caseNumber}</td>
                                <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">{c.patientName}</td>
                                <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{c.type}</td>
                                <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{c.policeStation}</td>
                                <td className="px-6 py-4">
                                    <span className={`px-2 py-1 rounded text-xs font-bold ${c.status === 'Active'
                                ...
                                <td className="px-6 py-4 text-right">
                                    <button type="button" className="text-teal-600 dark:text-teal-400 hover:underline" title="View File" aria-label="View file"><FileText size={18} /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
